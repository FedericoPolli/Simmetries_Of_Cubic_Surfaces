class TritangentPlane:
    def __init__(self, plane, lines_dict, sing_locus, conditions=None):
        ...
        if conditions is None:
            self.condition = self.find_conditions_for_eckardt_point()
        else:
    ...
    def find_conditions_for_eckardt_point(self):
        ...
        conditions = list(set(matrix([P1, P2]).minors(2)))
        final_conditions = []
        # remove sing factors and higher degree powers.
        for cond in conditions:
            if cond != 0:
                linear_conds_prod = prod([el[0] for el in list(cond.factor())])
                final_conditions.append(remove_sing_factors(linear_conds_prod, self.sing_locus))

        # it is enough to consider the gcd of the list of minors so obtained.
        # Uncomment the following lines to check if needed.
        # print(gcd(final_conditions))
        # print(P.ideal(final_conditions).radical().primary_decomposition())
        return gcd(final_conditions)

    def reduce(self, ideal, cl_lines, sing_locus):
        plane = ideal.reduce(self.plane)
        lines_dict = {key: cl_lines[key] for key in self.labels}
        conditions = ideal.reduce(self.condition)
        if conditions != 0:
    ...
    def update_with_sostitution(self, sost, classified_lines, sing_locus):
        plane = self._poly_ring(self.plane.subs(sost).numerator())
        lines_dict = {key: classified_lines[key] for key in self.labels}
        condition = self.condition.subs(sost)
        condition = self._poly_ring(condition * condition.denominator())
        if condition != 0:
            condition = remove_sing_factors(condition, sing_locus)
        return TritangentPlane(plane, lines_dict, sing_locus, condition)
